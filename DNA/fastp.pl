...
open Out,">$fOut.stat";
print Out join("\t","#sampleID","rawreads","rawdata","rawq20","rawq30","rawGC","adapterreads","cleanreads","cleandata","cleanq20","cleanq30","cleanGC","dup","rawread1_mean_length","rawread2_mean_length","cleanread1_mean_length","cleanread2_mean_length","low_quality_reads","N_reads"),"\n";
my @out;
push @out,$sample;
...
push @out,$obj->{"summary"}->{"before_filtering"}->{"read1_mean_length"};
push @out,$obj->{"summary"}->{"before_filtering"}->{"read2_mean_length"};
push @out,$obj->{"summary"}->{"after_filtering"}->{"read1_mean_length"};
push @out,$obj->{"summary"}->{"after_filtering"}->{"read2_mean_length"};
push @out,$obj->{"filtering_result"}->{"low_quality_reads"};
push @out,$obj->{"filtering_result"}->{"too_many_N_reads"};
print Out join("\t",@out),"\n";
close Out;
