...
while (<IN>) {
	...
	my ($chr,$pos,$ID,$REF,$ALT,$QUAL,$FILTER,$INFO,@gro) =split(/\s+/,$_);
	...
	foreach my $region (sort keys %{$region{$chr}}) {
		...
		if ($pos >= $pos3 && $pos <= $pos4){
			...
			my $i;
			for ($i=2;$i<@gro ;$i++) {
				my ($gt1,undef)=split(/\:/,$gro[1]);
				my ($gt2,undef)=split(/\:/,$gro[$i]);
				if ( $gt1 ne $gt2) {
					push @out,join("\t",join("||",$gt1,$gt2));
			...
			print Out join("\t",$region{$chr}{$region},$chr,$region,$ALT,@out),"\n";
		}
