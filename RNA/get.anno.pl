...
close In;
my @files=glob("$xls/*.enrichment.detail.xls");
#my @files=glob("$xls/*.pathway.xls");
my %allgene;
my @allname;
open Out,">$fout";
foreach my $file(@files){
	...
	while(<In>){
		...
		#print $all[0];die;
		#my $gen=$all[-1];
		#my @alname=split/\|/,$all[-2];
		my @alname=split/\;/,$all[-1];
		for (my $i=0;$i < scalar @alname ;$i++) {
			if(exists $gene{$alname[$i]}){
				print Out "$alname[$i]\t$all[-2]\t$all[0]\t$all[2]\t$all[-3]\n";
				#print Out "$alname[$i]\t$all[0]\t$all[2]\t$all[-4]\n";
